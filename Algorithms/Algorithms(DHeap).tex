...
\begin{itemize}[label=•, labelsep=1em]
    ...
    \textbf{Сложность:} $O(1)$ — проверка значения $d$ выполняется за постоянное время.
\end{itemize}
...
\begin{itemize}[label=•, labelsep=1em]
    ...
    \textbf{Сложность:}
    \begin{itemize}
        \item Добавление элемента в конец массива — $O(1)$.
        \item \texttt{SiftUp} имеет сложность $O(\log_d(n))$, так как высота $d$-арного дерева пропорциональна $\log_d(n)$.
    \end{itemize}
    \textbf{Итоговая сложность:} $O(\log_d(n))$.
\end{itemize}
...
\textbf{Сложность:}
\begin{itemize}
    \item Перемещение последнего элемента — $O(1)$.
    \item Уменьшение длины массива — $O(1)$.
    \item \texttt{SiftDown} имеет сложность $O(\log_d(n))$, так как высота $d$-арного дерева пропорциональна $\log_d(n)$.
\end{itemize}
\textbf{Итоговая сложность:} $O(\log_d(n))$.
...
\textbf{Сложность:} $O(\log_d(n))$, так как высота дерева пропорциональна $\log_d(n)$.
...
\begin{itemize}[label=•, labelsep=1em]
    ...
    \textbf{Сложность:} $O(d)$ — требуется проверить $d$ дочерних элементов.
\end{itemize}
